let handler = async (m, { text, conn, usedPrefix, command }) => {
	...
	if (!who) conn.reply(m.chat, exemplo, m, { mentions: [m.sender] })
	let res = [];
	console.log(command)
	switch (command) {
		case "blok":
		case "block":
			...
			else conn.reply(m.chat, exemplo, m, { mentions: [m.sender] })
		break
        case "unblok":
        case "unblock":
	...
	if (res[0]) conn.reply(m.chat, `\n   Sucesso ao executar ${command} em ${res.map(v => '@' + v.split("@")[0])}`, m, { mentions: res })
}
