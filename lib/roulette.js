...
export async function generateSlotSpinMp4(finalResult = ['💎', '💎', '💎']) {
    ...
    // --- NOVIDADE: Gera um ID único para o diretório temporário e o nome do arquivo MP4 ---
    const uniqueId = uuidv4(); // Gera um ID único
    const tempDir = path.join(__dirname, `temp_frames_${uniqueId}`); // Diretório temporário único
    const outputFileName = `slot_spin_${uniqueId}.mp4`; // Nome do arquivo MP4 único
    const finalOutputPath = path.join(tempDir, outputFileName); // Caminho completo para o MP4
    // --- FIM DA NOVIDADE ---
    ...
    return new Promise((resolve, reject) => {
        ffmpeg()
            ...
            .on('end', () => { // Removido 'async' e 'await rimraf' daqui
                console.log(`Vídeo MP4 gerado: ${finalOutputPath}`);
                resolve(finalOutputPath); // Retorna o caminho completo do arquivo
            })
            .on('error', (err) => { // Removido 'async' e 'await rimraf' daqui
