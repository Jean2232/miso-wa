...
function drawFrame(ctx, bgBuffer, images, currentFrame, sequences) {
    // --- NOVIDADE: Limpa o canvas para cada novo frame ---
...
export async function generateSlotSpinMp4(finalResult = ['💎', '💎', '💎']) {
    ...
    const tempDir = path.join(__dirname, 'temp_frames'); // Diretório temporário para os frames
    ...
    for (let frame = 0; frame <= maxFrames + holdFrames; frame++) {
        // Passa o contexto do bgBuffer para drawFrame, que agora limpa o próprio contexto
        drawFrame(bgBuffer.getContext('2d'), bgBuffer, images, frame, sequences);
        ...
        const stream = bgBuffer.createPNGStream();
        stream.pipe(out);
    ...
    return new Promise((resolve, reject) => {
        ffmpeg()
            ...
            .on('end', async () => {
                console.log(`Vídeo MP4 gerado: ${outputFileName}`);
                await rimraf(tempDir); // Limpa os frames temporários
                console.log('Arquivos temporários limpos.');
                resolve(outputFileName);
            })
            .on('error', async (err) => {
                console.error('Erro ao gerar vídeo MP4 com FFmpeg:', err.message);
                await rimraf(tempDir); // Tenta limpar mesmo em caso de erro
                reject(err);
            })
            .run();
    });
